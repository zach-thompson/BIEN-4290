...
do
    ...
    main_includes=$(grep "." files.txt | tr '\n' ' ') # create included files variable for executable target

    if (echo "$FILE" | grep -q ".cpp") # looks for .cpp files
    then      
        if (grep -q "main(" $FILE) # determines which .cpp contains a main function
        then
            echo "$FILE: $FILE $main_includes" | sed 's/.cpp/.o/' >> makefile # prints target.o: target.cpp main_includes.cpp
            executable=$(basename $1)
            echo -e "\t"'$(CC)' "-o" "$executable" '$^\n'  >> makefile # prints compile line with executable (name of folder)
        else
            echo "$FILE: $FILE $included" | sed 's/.cpp/.o/' >> makefile # prints target.o: target.cpp includes.cpp
