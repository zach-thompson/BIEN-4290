...
int main(int argc, char* argv[]){
	...
	//getting the path of the input data file from command line
	path = argv[1]; 
		
	std::cout << "Path of input file: "  << path << "\n";

	//Reading all the data points
	std::ifstream myFile(argv[1]);

	if(myFile.is_open())
	{
		//Constructor for the class
		Statistics::Stats anaylsisClass(min, max, mean, std);
		...
		std::cout << "Number of data points: " << n <<"\n";

		//Finding the minimum value of the file
		anaylsisClass.minimum(&input);
		std::cout << "Minimum: " << anaylsisClass.get_min() << "\n"; 	

		//Finding the max value of the file
		anaylsisClass.maximum(&input);
		std::cout << "Maximum: " << anaylsisClass.get_max() << "\n";

		//Calculating the mean value of the file
		anaylsisClass.meanCalc(&input);
		std::cout << "Mean: " << anaylsisClass.get_mean() << "\n";

		//Calculating the standard deviation of the file
		anaylsisClass.standardDev(&input, anaylsisClass.get_mean(), n);
		std::cout << "Standard deviation: " << anaylsisClass.get_std() << "\n";

		//Writing the Histogram to the terminal
		output = anaylsisClass.histogram(&input, anaylsisClass.get_std(), anaylsisClass.get_mean());
		std::cout << "Histogram for file:\n";
	...
	{
		std::cout << "ERROR: The file was upable to be opened!\n";
	}
